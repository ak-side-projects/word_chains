...
module WordChainer
  ...
  def run(source, target, dictionary_file_name)
    ...
    parents && build_path(parents, source, target, dictionary)
  end

  def build_path(parents, source, target, dictionary)
    chain_word = target
    path = []
  ...
  def adjacent_words(word, dictionary)
    ...
    word.each_char.with_index do |old_letter, i|
      ('a'..'z').each do |new_letter|
        ...
        word_copy = word.dup
        word_copy[i] = new_letter

        adjacent_words << word_copy if dictionary.include?(word_copy)
      end
    end
...
if __FILE__ == $PROGRAM_NAME
  p WordChainer.run("frog", "golf", "dictionary.txt")
end